...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Adn: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Adn Signed Message:\n";
...
{
    // Adn: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Adn: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30; // Adn: 1/2 minutes
static const int64 nTargetSpacing = 1 * 10; // Adn: 10 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Adn: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Adn: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xf3, 0xa2, 0xd5, 0xbf }; // Adn: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdnMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdnMiner(CWallet *pwallet)
{
    printf("AdnMiner started\n");
    ...
    try { loop {
        ...
        printf("Running AdnMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdnMiner, pwallet));
}
